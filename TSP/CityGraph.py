...
# 80% of possible connection
def graph_to_edge_list(graph):
    edge_list = []
    for city, neighbors in graph.items():
        for neighbor, distance in neighbors:
            edge_list.append((city, neighbor, distance))
    return edge_list

def remove_20_percent_edges(edge_list):
    num_edges_to_keep = int(len(edge_list) * 0.8)
    random.shuffle(edge_list)
    return edge_list[:num_edges_to_keep]

def edge_list_to_graph(edge_list, num_cities):
    reduced_graph = {i: [] for i in range(num_cities)}
    for city1, city2, distance in edge_list:
        reduced_graph[city1].append((city2, distance))
        reduced_graph[city2].append((city1, distance))
